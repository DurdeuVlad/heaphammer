...
/** Profile-aware live entity operations for Fabric 1.21.1. */
public final class FabricEntityLifecyclePort implements EntityLifecyclePort {
    ...
    public FabricEntityLifecyclePort(Supplier<MinecraftServer> serverSupplier) {
    ...
    public UUID spawn(String dimension, String entityTypeId, double x, double y, double z, EntityWorkloadProfile profile) {
        ...
        entity.addTag(FabricPlatformAdapter.TEST_ENTITY_TAG);
    ...
    public boolean cycleChunk(String dimension, int chunkX, int chunkZ, EntityWorkloadProfile profile) {
        // Fabric 1.21.1 has no stable public unload/reload contract for an individual
        // live entity chunk. Report this as unsupported rather than silently simulating it.
        return false;
    }
    ...
    public long countMaterializedTestEntities(String dimension) {
        ...
        for (Entity entity : level.getAllEntities()) {
            if (entity.getTags().contains(FabricPlatformAdapter.TEST_ENTITY_TAG)) count++;
        }
        return count;
    }
